...
class WpTextEditor( wx.stc.StyledTextCtrl ):
	...
	def _OnKeyDown( self, event ):
		...
		if key == wx.WXK_NUMPAD_ENTER or key == wx.WXK_RETURN:
			## take care of currentline
			currentLine = self.GetCurrentLine()
			currentIndentation = self.GetLineIndentation( currentLine )
			currentTabWidth = self.GetTabWidth()

			## If colon, add one extra tab
			if self.GetCharAt( self.GetCurrentPos()-1 ) == 58:
				currentIndentation += currentTabWidth

			self.NewLine()
			self.SetLineIndentation( currentLine+1, ( currentIndentation ) )
			self.LineEnd()
		else:
			event.Skip()
